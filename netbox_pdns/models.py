...
class Settings(BaseSettings):
    ...
    webhook_secret: str | None = Field(
        default=None,
        description=(
            "Secret for HMAC webhook signature verification. If not set, only API key auth is used."
        ),
    )
    ...
    def validate_pdns_url(cls, v: str) -> str:
        ...
        if parsed.scheme not in {"http", "https"}:
            raise ValidationError(
                f"PowerDNS URL must use http or https scheme, got: {parsed.scheme}"
            )
    ...
    def validate_mqtt_broker_url(cls, v: str) -> str:
        ...
        if parsed.scheme not in {"mqtt", "mqtts"}:
            raise ValidationError(
                f"MQTT broker URL must use mqtt or mqtts scheme, got: {parsed.scheme}"
            )
    ...
    def validate_mqtt_client_id(cls, v: str) -> str:
        ...
        if not re.match(r"^[a-zA-Z0-9_-]+$", v.strip()):
            raise ValidationError(
                "MQTT client ID can only contain alphanumeric characters, dashes, and underscores"
            )
    ...
    def validate_mqtt_topic_prefix(cls, v: str) -> str:
        ...
        if not re.match(r"^[a-zA-Z0-9_/-]+$", cleaned):
            raise ValidationError(
                "MQTT topic prefix can only contain alphanumeric characters, "
    ...
    def validate_mqtt_auth(self) -> "Settings":
        ...
        if self.mqtt_enabled:
            has_username = self.mqtt_username is not None and bool(
                self.mqtt_username.strip() if self.mqtt_username else False
            )
            has_password = self.mqtt_password is not None and bool(
                self.mqtt_password.strip() if self.mqtt_password else False
            )

            if has_username != has_password:
                raise ConfigurationError(
                    "Both mqtt_username and mqtt_password must be provided together, "
                    "or both omitted"
                )
